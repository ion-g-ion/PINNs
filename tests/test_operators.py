...
def test_gradient():
    ...
    assert error < 1e-13, "pinns.operators.gradient error: wrong gradient."
...
def test_gradient2():
    ...
    error = jnp.linalg.norm(gc - gr)
    assert error < 1e-13, "pinns.operators.gradient error: wrong gradient for 2 arguments (first wrong)."

    gc = grad_computedy(x, y)
    gr = grad_referencey(x, y)
    error = jnp.linalg.norm(gc - gr)
    assert error < 1e-13, "pinns.operators.gradient error: wrong gradient for 2 arguments (second wrong)."
...
def test_laplace():
    ...
    assert error < 1e-13, "pinns.operators.lapalce error: wrong laplace."
...
def test_lapalce2():
    ...
    error = jnp.linalg.norm(lc - lr)
    assert error < 1e-13, "pinns.operators.lapalce error: wrong laplace for 2 arguments (first wrong)."

    lr = laplace_referencey(x, y)
    lc = laplace_computedy(x, y)
    error = jnp.linalg.norm(lc - lr)
    assert error < 1e-13, "pinns.operators.lapalce error: wrong laplace for 2 arguments (second wrong)."
...
def test_divergence():
    ...
    assert error < 1e-13, "pinns.operators.divergence error: wrong divergence."
...
def test_curl2d():
    ...
    assert error < 1e-13, "pinns.operators.curl2d error: wrong curl."
...
def test_curl3d():
    ...
    error = jnp.linalg.norm(cc - cr)
    assert error < 1e-13, "pinns.operators.curl3d error: wrong curl."
...
def test_jacobian():
    ...
    error = jnp.linalg.norm(jc - jr)
    assert error < 1e-13, "pinns.operators.jacobian error: wrong jacobian."