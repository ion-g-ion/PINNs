...
class Model(pinns.PINN):
    def __init__(self, rand_key):
        ...
        self.add_neural_network('u1',stax.serial(block,block,stax.Dense(1)),(-1,2)) # iron
        self.add_neural_network('u4',stax.serial(block,block,stax.Dense(1)),(-1,2)) # iron 2
        self.add_neural_network('u2',stax.serial(block,block,stax.Dense(1)),(-1,2)) # air 
        self.add_neural_network('u3',stax.serial(block,block,stax.Dense(1)),(-1,2)) # copper
        ...
        self.add_neural_network('u14',stax.serial(stax.Dense(nl),stax.Tanh,stax.Dense(nl),stax.Tanh,stax.Dense(1)),(-1,1))
        self.add_neural_network('u34',stax.serial(stax.Dense(nl),stax.Tanh,stax.Dense(nl),stax.Tanh,stax.Dense(1)),(-1,1))
    ...
    def solution1(self, ws, x):
        ...
        w =  self.interface12(ws['u12'],x)*((x[...,0] - 1)*(x[...,0] + 0))[...,None] + self.interface13(ws['u13'],x)*(x[...,1] - 1)[...,None]*(x[...,1] + 0)[...,None] +  self.interface14(ws['u14'],x) * ((x[...,0] - 1)*(x[...,0] + 0))[...,None]
    ...
    def solution2(self, ws, x):
        ...
        w = self.interface21(ws['u12'],x)*((x[...,1] - 1)*(x[...,1] + 0))[...,None] + self.interface23(ws['u23'],x)*(x[...,0] - 1)[...,None]
    ...
    def solution3(self, ws, x):
        ...
        v = ((x[...,1] - 1)*(x[...,1] + 0)*(x[...,0]-1))[...,None]
        w =  self.interface32(ws['u23'],x)*(x[...,0] - 1)[...,None]+self.interface31(ws['u13'],x)*(x[...,1] - 1)[...,None]*(x[...,1] + 0)[...,None]+self.interface34(ws['u34'],x)*(x[...,0] - 1)[...,None]
        w = w + ws['u123']*( x[...,0] * x[...,1] )[...,None] + ws['u134'] *  ( (x[...,0] - 0)*(1-x[...,1]) )[...,None]
        return u*v + w
    ...
    def solution4(self, ws, x):
        ...
        w = self.interface41(ws['u14'],x)*((x[...,0] - 1)*(x[...,0] + 0))[...,None]+self.interface43(ws['u34'],x)*((x[...,1] - 1))[...,None]
    ...
    def loss_pde(self, ws):
        ...
        lpde3 = 0.5*self.mu0*jnp.dot(jnp.einsum('mi,mij,mj->m',grad3,self.points['K3'],grad3), self.points['ws'])  - jnp.dot(self.J0*self.neural_networks['u3'](ws['u3'],self.points['ys']).flatten()*self.points['omega3']  ,self.points['ws'])
