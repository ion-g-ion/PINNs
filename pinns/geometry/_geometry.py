...
class PatchParametrized(Patch):
    ...
    def __init__(self, parametrization, dims, rand_key):
        ...
        self._parametrization_bd_jacobian = jax.jit(
            jax.jacfwd(parametrization))
...
class PatchNURBS(Patch):
    ...
    def __call__(self, y, differential=False):
        ...
        xs = np.einsum(
            'im,i...->m...', Bs[0], np.einsum('...i,...->...i', self.knots, self.weights))
        for i in range(1, self.d):
        ...
        if differential:
            ...
            if self.d == 3 and self.dembedding == 3:
                diff = np.abs(DGys[:, 0, 0]*DGys[:, 1, 1]*DGys[:, 2, 2] + DGys[:, 0, 1]*DGys[:, 1, 2]*DGys[:, 2, 0]+DGys[:, 0, 2]*DGys[:, 1, 0]*DGys[:, 2, 1] - DGys[:, 0, 2]
                              * DGys[:, 1, 1]*DGys[:, 2, 0] - DGys[:, 0, 0]*DGys[:, 1, 2]*DGys[:, 2, 1] - DGys[:, 0, 1]*DGys[:, 1, 0]*DGys[:, 2, 2])*Weights  # type: ignore
    ...
    def _eval_derivative(self, y, dim):
        ...
        xs = jnp.einsum(
            'im,i...->m...', Bs[0], jnp.einsum('...i,...->...i', self.knots, self.weights))
        for i in range(1, self.d):
            xs = jnp.einsum('im,mi...->m...', Bs[i], xs)

        Dxs = jnp.einsum(
            'im,i...->m...', dBs[0], jnp.einsum('...i,...->...i', self.knots, self.weights))
        for i in range(1, self.d):
    ...
    def importance_sampling(self, N, pdf=None):
        ...
        ys = np.random.rand(
            N, self.d)*np.array([i[1]-i[0] for i in self.bounds]) + np.array([i[0] for i in self.bounds])
